...
use openclaw_agent::ports::{
    AIPort, MemoryPort, SecurityPort, ToolPort, DevicePort,
    MemoryEntry, SecurityCheckResult, ToolInfo, RecallItem,
    CameraInfo, ScreenInfo, LocationInfo,
};
use openclaw_core::{Message, Role, Content, OpenClawError};
use openclaw_core::Result as OpenClawResult;
use openclaw_ai::{AIProvider, ChatRequest, ChatResponse, EmbeddingRequest, EmbeddingResponse, StreamChunk};
use openclaw_memory::MemoryManager;
use openclaw_memory::types::{MemoryRetrieval, MemoryItem, MemoryContent};
use openclaw_memory::recall::{RecallResult as MemoryRecallResult, RecallItem as MemoryRecallItem};
...
impl SecurityPort for SecurityPortAdapter {
    async fn check(&self, _input: &str) -> OpenClawResult<SecurityCheckResult> {
        Ok(SecurityCheckResult { allowed: true, reason: None })
    }
}
...
impl ToolPort for ToolPortAdapter {
    async fn execute(
        ...
        let skill_found = all_skills.iter().any(|s| s.id == tool_name || s.name == tool_name);
        ...
        if skill_found {
            ...
            Err(OpenClawError::Tool(format!("Tool '{}' not found or not available", tool_name)))
        }
    }
...
impl DevicePort for DevicePortAdapter {
    async fn list_cameras(&self) -> OpenClawResult<Vec<CameraInfo>> {
        ...
        let cameras: Vec<CameraInfo> = capabilities.iter()
    ...
    async fn capture_camera(&self, camera_id: &str, path: &str) -> OpenClawResult<String> {
        let _result = self.manager.capture_camera(camera_id).await
    ...
    async fn list_screens(&self) -> OpenClawResult<Vec<ScreenInfo>> {
        let capabilities = self.manager.list_capabilities().await;
        let screens: Vec<ScreenInfo> = capabilities.iter()
    ...
    async fn capture_screen(&self, screen_id: &str, path: &str) -> OpenClawResult<String> {
        let _result = self.manager.capture_screen(screen_id).await
            .map_err(|e| OpenClawError::Config(e.to_string()))?;
        Ok(path.to_string())
    }
