...
impl KimiProvider {
    ...
    fn get_base_url(&self) -> &str {
        self.config.base_url.as_deref().unwrap_or("https://api.moonshot.cn/v1")
    }
...
impl AIProvider for KimiProvider {
    ...
    async fn chat(&self, request: ChatRequest) -> Result<ChatResponse> {
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| OpenClawError::Http(format!("Kimi API 请求失败: {}", e)))?;

        if !response.status().is_success() {
            let error_text = response.text().await.unwrap_or_default();
            return Err(OpenClawError::AIProvider(format!("Kimi API 错误: {}", error_text)));
        }
    ...
    async fn chat_stream(
        ...
        Err(OpenClawError::AIProvider(
            "Streaming not yet implemented for Kimi".to_string()
        ))
    }
    ...
    async fn embed(&self, request: EmbeddingRequest) -> Result<EmbeddingResponse> {
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| OpenClawError::Http(format!("Kimi Embedding API 请求失败: {}", e)))?;

        if !response.status().is_success() {
            let error_text = response.text().await.unwrap_or_default();
            return Err(OpenClawError::AIProvider(format!("Kimi Embedding API 错误: {}", error_text)));
        }
