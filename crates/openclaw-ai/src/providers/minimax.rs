...
impl AIProvider for MinimaxProvider {
    ...
    async fn chat(&self, request: ChatRequest) -> Result<ChatResponse> {
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| OpenClawError::Http(format!("Minimax API 请求失败: {}", e)))?;

        if !response.status().is_success() {
            let error_text = response.text().await.unwrap_or_default();
            return Err(OpenClawError::AIProvider(format!("Minimax API 错误: {}", error_text)));
        }
    ...
    async fn chat_stream(
        ...
        Err(OpenClawError::AIProvider(
            "Streaming not yet implemented for Minimax".to_string()
        ))
    }
    ...
    async fn embed(&self, request: EmbeddingRequest) -> Result<EmbeddingResponse> {
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| OpenClawError::Http(format!("Minimax Embedding API 请求失败: {}", e)))?;

        if !response.status().is_success() {
            let error_text = response.text().await.unwrap_or_default();
            return Err(OpenClawError::AIProvider(format!("Minimax Embedding API 错误: {}", error_text)));
        }
