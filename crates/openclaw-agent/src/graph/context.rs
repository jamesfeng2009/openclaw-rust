...
impl GraphResponse {
    pub fn from_context(context: GraphContext, end_nodes: &[String]) -> Self {
        ...
        let result = final_node
            .and_then(|n| context.results.get(n))
...
mod tests {
    use super::*;
    use crate::graph::definition::{EdgeDef, GraphDef, NodeDef, NodeType};
    ...
    fn test_graph_response_from_context() {
        ...
        context.results.insert(
            "end".to_string(),
            NodeResult::success("end", serde_json::json!({"result": "done"}), 100),
        );
    ...
    fn test_context_shared_data() {
        ...
        assert_eq!(
            context.shared_data.get("key1").unwrap(),
            &serde_json::json!("value1")
        );
        assert_eq!(
            context.shared_data.get("key2").unwrap(),
            &serde_json::json!(42)
        );
    }
    ...
    fn test_set_and_get_shared_value() {
        ...
        assert_eq!(
            context.get_shared_value("test_key"),
    ...
    fn test_update_shared_value() {
        ...
        assert_eq!(
            context.get_shared_value("counter"),
