...
impl GraphPatterns {
    pub fn sequential(agent_ids: &[&str]) -> GraphDef {
        ...
        for (i, agent_id) in agent_ids.iter().enumerate() {
            ...
            if i == 0 {
                graph = graph.with_node(NodeDef::new("start", NodeType::Router).with_name("Start"));
        ...
        graph = graph.with_node(NodeDef::new(&end_id, NodeType::Terminal).with_name("End"));
    ...
    pub fn broadcast(agent_ids: &[&str]) -> GraphDef {
        ...
        graph = graph.with_node(NodeDef::new("start", NodeType::Router).with_name("Broadcaster"));
        graph = graph.with_start("start".to_string());
    ...
    pub fn tree(depth: usize, branching: usize, agent_ids: &[&str]) -> GraphDef {
        ...
        graph = graph.with_node(NodeDef::new("root", NodeType::Router).with_name("Root"));
    ...
    pub fn map_reduce(map_agents: &[&str], reduce_agent: &str) -> GraphDef {
        ...
        graph = graph.with_node(NodeDef::new("start", NodeType::Router).with_name("Splitter"));
        graph = graph.with_start("start".to_string());
    ...
    pub fn mixture_of_experts(expert_agents: &[&str]) -> GraphDef {
        ...
        graph =
            graph.with_node(NodeDef::new("router", NodeType::Router).with_name("Expert Router"));
