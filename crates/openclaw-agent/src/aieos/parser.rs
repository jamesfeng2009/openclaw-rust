...
impl AIEOSParser {
    ...
    pub fn validate(aieos: &AIEOS) -> Result<()> {
        if aieos.version.is_empty() {
            return Err(OpenClawError::Config("AIEOS version is required".to_string()));
        }

        if aieos.identity.names.first.is_empty() {
            return Err(OpenClawError::Config("Agent name (identity.names.first) is required".to_string()));
        }

        if aieos.psychology.neural_matrix.creativity > 1.0 
            || aieos.psychology.neural_matrix.creativity < 0.0 {
            return Err(OpenClawError::Config("Neural matrix values must be between 0 and 1".to_string()));
        }

        if aieos.linguistics.text_style.formality_level > 1.0 
            || aieos.linguistics.text_style.formality_level < 0.0 {
            return Err(OpenClawError::Config("Formality level must be between 0 and 1".to_string()));
        }
